...
- [cosmic-ray works native on Windows, mutmut needs WSL](windwright-cosmic-ray-works-native-windows-not-wsl.md) — verified directly tonight (real mutation runs, not assumed); also flags two new Windwright governance scripts (locked-constant guard, tool-registry drift checker) that fit Lawkeeper's own category and have a real precedent (ai_review.py, merge_gate.py) for porting over.
