...
| **Windwright real-vs-claimed audit** (new 2026-08-21) | The user's own honest assessment tonight: Windwright was assumed the most-developed repo, but a single evening of actual scrutiny found an under-constrained optimizer producing suspiciously-perfect-but-fake results across the entire production path, ~50-71% dead/fabricated YouTube resource links, and a `match_timbre`/`design_from_sound` capability never demonstrated against real audio -- the user's stated expectation is "plenty more bugs yet to be discovered." A one-off cleanup pass is not enough; this needs to be a real, recurring check, not a single sweep. | Weekly, alongside the compliance sweep above, but scoped specifically to "does this claimed-working feature actually work," not just style/mutability checks | Manual/on-demand headless run (needs real judgment on what to test, same as tonight's audio pipeline debugging) |
