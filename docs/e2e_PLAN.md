...
Status: **APPROVED** by user (build mode). Defaults applied unless user edits.
...
1. `tests/test_e2e.py` is collected by `pytest tests/ -q`.
2. Each scenario passes on this machine (git 2.55) and on Linux CI.
...
Note: plan files live in `docs/` and repo root (NOT `tests/`, which is `.py` only
per the placement rule enforced by scripts/validate_pre_commit.py).
